...
import { onPointTransactionCreated } from '../activities/triggers/pointTransactionCreated';
...
describe('onPointTransactionCreated Integration Test', () => {
  ...
  beforeAll(() => {
    ...
    db = admin.firestore();
    wrapped = testEnv.wrap(onPointTransactionCreated);
  });
  ...
  it('safely increments student points and generates an activity, rejecting duplicate events', async () => {
    ...
    expect(updatedStudent.data()?.totalPoints).toBe(15);
    ...
    expect(unchangedStudent.data()?.totalPoints).toBe(15);
